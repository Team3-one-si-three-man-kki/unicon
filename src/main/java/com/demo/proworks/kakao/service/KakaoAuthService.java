...
public class KakaoAuthService {
	 ...
	 * 카카오 로그인 URL 생성
	 */
	public String getKakaoLoginUrl(String serviceCode, String tenant) throws UnsupportedEncodingException {
		String state = URLEncoder.encode(tenant, StandardCharsets.UTF_8.name());
		String fullRedirectUri = kakaoRedirectUri + "/" + serviceCode + ".pwkjson";

		return UriComponentsBuilder.fromUriString("https://kauth.kakao.com/oauth/authorize")
				.queryParam("client_id", kakaoClientId).queryParam("redirect_uri", fullRedirectUri)
				.queryParam("response_type", "code").queryParam("state", state).build().toUriString();
	}
