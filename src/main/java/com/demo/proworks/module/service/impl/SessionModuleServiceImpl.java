...
public class SessionModuleServiceImpl implements SessionModuleService {
	 ...
	 * 세션모듈설정정보 목록을 조회합니다.
	 *
	 * @process 1. 세션모듈설정정보 페이징 처리하여 목록을 조회한다. 2. 결과 List<SessionModuleVo>을(를) 리턴한다.
	 * 
	 ...
	 * 조회한 세션모듈설정정보 전체 카운트
	 *
	 * @process 1. 세션모듈설정정보 조회하여 전체 카운트를 리턴한다.
	 * 
	 * @param sessionModuleVo 세션모듈설정정보 SessionModuleVo
	 ...
	 * 세션모듈설정정보를 상세 조회한다.
	 *
	 * @process 1. 세션모듈설정정보를 상세 조회한다. 2. 결과 SessionModuleVo을(를) 리턴한다.
	 * 
	 * @param sessionModuleVo 세션모듈설정정보 SessionModuleVo
	 * @return 단건 조회 결과
	 * @throws Exception
	 ...
	 *
	 * @process 1. 특정 세션ID로 해당 세션의 모듈 상세 목록을 조회한다. 2. 결과
	 *          List<SessionModuleDetailVo>을(를) 리턴한다.
	 * 
	...
	public List<SessionModuleDetailVo> selectListSessionModuleDetailBySessionId(String sessionId) throws Exception {
		List<SessionModuleDetailVo> list = sessionModuleDAO.selectListSessionModuleDetailBySessionId(sessionId);
		return list;
	}
}
